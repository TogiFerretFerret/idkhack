package sh.idktheflag.idkhack.api.management.network;

import sh.idktheflag.idkhack.api.utils.discord.DiscordEventHandlers;
import sh.idktheflag.idkhack.api.utils.discord.DiscordRPC;
import sh.idktheflag.idkhack.api.utils.discord.DiscordRichPresence;
...
import sh.idktheflag.idkhack.impl.features.modules.client.RPC;
...
public class DiscordPresence implements IMinecraft {
        ...
        thread = new Thread(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    System.err.println("[idkhack] Discord IPC error: " + e.getMessage());
                    closeSocket();
    ...
    private static void startWindows() {
        ...
        if (rpc == null) {
            System.out.println("[idkhack] Discord RPC native library not available, skipping");
            return;
        }
        if (thread != null) thread.interrupt();
