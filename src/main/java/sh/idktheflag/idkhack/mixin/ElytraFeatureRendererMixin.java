...
import sh.idktheflag.idkhack.impl.features.modules.render.Capes;
...
public abstract class ElytraFeatureRendererMixin<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M>
{
    ...
    private Identifier modifyCapeTexture(Identifier original, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l)
    {
        ...
        {
            if (playerEntity.getGameProfile() != null && playerEntity.getGameProfile().getId() != null)
            {
                if (MinecraftClient.getInstance().getSession().getUuidOrNull().toString().equals(playerEntity.getGameProfile().getId().toString()))
                {
                    return Identifier.of("kami", "capes/" + Capes.INSTANCE.capeMode.getValue().toLowerCase() + ".png");
                } else
                {
//                    String cape = BotManager.INSTANCE.getCapeForName(playerEntity.getGameProfile().getName());
//               1     if (cape != null)
//                    {
