...
public class SavableManager implements IMinecraft
{
    ...
    public static final File MAIN_FOLDER = new File(System.getProperty("user.dir") + File.separator + "idk");
    ...
    {
        File dir = new File(mc.runDirectory.getAbsolutePath(), File.separator + "idk" + File.separator);
        if (!dir.exists())
        {
            dir.mkdir();
        }
        File namedFile = new File(mc.runDirectory.getAbsolutePath(), File.separator + "idk" + File.separator + name + "." + exstension);
        if (!namedFile.exists())
        {
