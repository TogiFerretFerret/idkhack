...
public class FastProjectile extends Module {
        ...
        for (int i = 0; i < strength.getValue().floatValue(); i++)
        {
            switch (mode.getValue())
            {
                ...
                case "Stonian":
                    ...
                    movePacket(mc.player.getX(), mc.player.getY() + 0.00000000001f, mc.player.getZ(), false);
                    break;
                //forgot tbh, just pasting this one over from idk cuz i think it bypassed some server cloudanarchy or something
                case "Float":
    ...
    {
        if (rotate.getValue())
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(x, y, z, mc.player.getYaw(), mc.player.getPitch(), ground, mc.player.horizontalCollision));
        else
