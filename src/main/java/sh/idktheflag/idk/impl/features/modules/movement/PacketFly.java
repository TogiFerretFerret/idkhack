...
import sh.idktheflag.idk.api.event.events.network.ServerEvent;
...
public class PacketFly extends Module
{
    ...
    public void onPacketReceive(PacketEvent.Receive event)
    {
        ...
        if (event.getPacket() instanceof PlayerPositionLookS2CPacket packet)
        {
            ...
            // We can't modify the packet (record), so we just use the teleport ID
            PacketManager.INSTANCE.sendQuietPacket(new TeleportConfirmC2SPacket(packet.teleportId()));
    ...
    public void onPacket(PacketEvent.Send event)
    {
        ...
        if (event.getPacket() instanceof TeleportConfirmC2SPacket || event.getPacket() instanceof PlayerMoveC2SPacket) {
             // In singleplayer we cancel to prevent rubberband?
             // Actually, the original code had !mc.isInSingleplayer() return
             // So it ONLY cancels in singleplayer. Weird.
