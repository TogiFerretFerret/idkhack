...
public class FakePlayer extends Module
{
    ...
    public void onExplosion(ExplosionS2CPacket explosion)
    {
        if ((fakePlayer.hurtTime == 0 || crystalAvailable))
        {
            ...
            if (!crystalAvailable)
                fakePlayer.onDamaged(mc.world.getDamageSources().generic());
            fakePlayer.setHealth((float) ((fakePlayer.getHealth() + fakePlayer.getAbsorptionAmount()) - sh.idktheflag.idk.api.utils.world.CrystalUtil.calculateDamage(fakePlayer, explosion.center(), false, false)));
